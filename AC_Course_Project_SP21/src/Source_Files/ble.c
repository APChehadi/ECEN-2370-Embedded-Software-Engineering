...
/***************************************************************************//**
 * @brief
 *   sets up and LEUART_OPEN_STRUCT and calls leuart_open() with the information
 *
 * @details
 *      sets the values of the LEUART_OPEN_STRUCT using defines from ble.h, including
 *      baudrate, databits, enable, parity, stopbits, as well as the tx and rx routes
 *      and enables for LEUART0
 *
 * @note
 *     this code is to specify to the system to use LEUART0
 *
 * @param[in] tx_event
 *   Event callback define for when transmission is done
 *
 * @param[in] rx_event
 *   Event callback define for when receiving is done
 *
 ******************************************************************************/
void ble_open(uint32_t tx_event, uint32_t rx_event){
	LEUART_OPEN_STRUCT leuart_opn;

	leuart_opn.baudrate = HM10_BAUDRATE;
	leuart_opn.databits = HM10_DATABITS;
	leuart_opn.enable = HM10_ENABLE;
	leuart_opn.parity = HM10_PARITY;
	leuart_opn.stopbits = HM10_STOPBITS;

	leuart_opn.tx_loc = LEUART0_TX_ROUTE;
	leuart_opn.rx_loc = LEUART0_RX_ROUTE;

	leuart_opn.tx_pin_en = LEUART_ROUTEPEN_TXPEN;
	leuart_opn.rx_pin_en = LEUART_ROUTEPEN_RXPEN;

	leuart_opn.tx_en = true;
	leuart_opn.rx_en = true;

	leuart_opn.tx_done_evt = tx_event;
	leuart_opn.rx_done_evt = rx_event;

	leuart_open(LEUART0, &leuart_opn);
}
 ...
 *
 * @details
 *      calls leuart_start() with LEUART0, the string that was passed in, the length
 *      of that string, and the BLE_TX_DONE_CB from app.h (the purpose is to keep the leuart
 *      driver general, so we have to send it the specific LEUART we want to use, as well as the
 *      strings and events)
 *
 * @note
 *     no output
 *
 * @param[in] *string
 *   pointer to the string to pass into leuart_start()
 *
 ******************************************************************************/
...
bool ble_test(char *mod_name){
	...
	// ANSWER: The board can sleep until an event occurs with interrupts. Polling requires the CMU to always be on.
	// How does interrupts benefit the system for low energy operation?
	// ANSWER: The board can be sleeping when an interrupt does not occur. Only when an interrupt occurs does the system have to be activated.
	// How does interrupts benefit the system that has multiple tasks?
	// ANSWER: The system can be working on something else and then temporarily deal with a new interrupt. There are nested interrupts and a hierarchy.
...
//	leuart_cmd_write(HM10_LEUART0, (LEUART_CMD_CLEARRX | LEUART_CMD_CLEARTX));
	...
	// DSD HM10 module?
	// ANSWER: Bluetooth cannot be connected while the name is being changed. Next time the device re-connects,
	// the correct name will be transfered the devices can communicate.
