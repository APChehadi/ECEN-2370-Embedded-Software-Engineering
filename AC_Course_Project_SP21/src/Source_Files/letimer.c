 ...
 * 	January 12th, 2020
 * @brief
 * 	Contains all the LETIMER driver functions
 *
 */
...
//	letimer->IFC = _LETIMER_IFC_MASK;
	...
	if(app_letimer_struct->comp0_irq_enable) {
		letimer->IFC &= ~app_letimer_struct->comp0_cb;
	...
	// Configure static CallBack variables
 ...
 *
 * @details
 * 	 Passes the enable bit and LETIMER0 peripheral from app.c, allowing the LETIMER to start once the initial setup has been completed.
 *
 * @note
 *   The enabling/disabling of the LETIMER modifies the LETIMER CMD register which requires synchronization into the low-frequency domain.
 *   If this register is modified before a previous update to the same register has completed, this function will stall until the previous
...
/***************************************************************************//**
 * @brief
 *	LETIMER0 Interrupt Request Handler
 *
 * @details
 *	This function call will set the callbacks to be serviced.
 *
 * @note
